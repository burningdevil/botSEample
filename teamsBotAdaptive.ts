        ...
        this.onMessage(async (context, next) => {
            if (context.activity.text != null) {
                ...
                if (text.includes("Card Actions")) {
                    ...
                    await context.sendActivity(
                        "Please use one of these commands: **Card Actions** for  Adaptive Card Actions, **Suggested Actions** for Bot Suggested Actions and **ToggleVisibility** for Action ToggleVisible Card"
                    );
                }
            }
        ...
        for (const idx in activity.membersAdded) {
            if (activity.membersAdded[idx].id !== activity.recipient.id) {
                const welcomeMessage =
                    `Welcome to Adaptive Card Action and Suggested Action Bot. This bot will introduce you to suggested actions.` +
                    "Please select an option:";

                await turnContext.sendActivity(welcomeMessage);
                await turnContext.sendActivity(
                    "Please use one of these commands: **1** for  Adaptive Card Actions, **2** for Bot Suggested Actions and **3** for Toggle Visible Card"
                );
